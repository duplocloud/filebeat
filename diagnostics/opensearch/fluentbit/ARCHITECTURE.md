...
- **Index prefix** — builds `index_prefix` for per-tenant index routing. The output
  ...
  correct index (e.g. `filebeat-fluentbit-2026.03.13`).
...
| `SERVICE_LEVEL_INDEX=yes` | `{INDEX_PREFIX}{container}-YYYY.MM.DD`          |
| Default                   | `filebeat-fluentbit-YYYY.MM.DD`                 |

The `INDEX_PREFIX` env var defaults to `filebeat-fluentbit-`, producing indices like
`filebeat-fluentbit-2026.03.13`. All tenants share one daily index by default. The
`filebeat-` prefix is intentional — it ensures indices are matched by the pre-existing
`filebeat-*` index pattern in OpenSearch Dashboards, providing backwards compatibility
with existing dashboards, saved searches, and queries without requiring a new index
pattern. To split indices per service, set `SERVICE_LEVEL_INDEX=yes`.
...
```json
{
  ...
  "tenant": { "name": "dev", "id": "" },
  "agent": { "name": "fluentbit-dev-ds-x222z", "type": "fluent-bit", "version": "4.2.4", "hostname": "fluentbit-dev-ds-x222z" },
  "host": { "name": "fluentbit-dev-ds-x222z" },
  "input": { "type": "container" },
  "index_prefix": "filebeat-fluentbit"
}
```
...
Two modes are supported. The Lua script, fluent-bit.conf, and Docker image are identical
in both modes — only the scheduling and placement layer differs.
...
- **nodeSelector** uses `tenantname: <namespace>` to schedule pods only on nodes owned
  by that tenant.
- **`duplocloud.net/daemnonset-tenant-local: "True"`** — tells DuploCloud the DaemonSet
  is scoped to this tenant.

### Cluster-wide (`--cluster-wide`)

- **Single DaemonSet for the entire cluster.** Schedules on every Linux node regardless
  of tenant ownership, collecting logs from all namespaces.
- **No `tenantname` nodeSelector** — only `kubernetes.io/os: linux` is applied.
- **Catch-all toleration** (`operator: Exists`) — ensures pods schedule on nodes with
  any taint (spot instances, control-plane, custom taints).
- **`duplocloud.net/daemnonset-tenant-local: "False"`** — signals to DuploCloud that the
  DaemonSet is not tenant-scoped.
- Resource names use `fluentbit-cluster-*` instead of `fluentbit-<namespace>-*`.

| Aspect | Tenant-scoped | Cluster-wide |
| ------ | ------------- | ------------ |
| nodeSelector | `tenantname: <ns>` | *(none — all Linux nodes)* |
| Tolerations | spot-instance only | all taints (`operator: Exists`) |
| Annotation | `tenant-local: "True"` | `tenant-local: "False"` |
| Resource names | `fluentbit-<namespace>-*` | `fluentbit-cluster-*` |
| Use case | Parallel testing alongside Filebeat | Full cluster rollout replacing Filebeat |

### RBAC (both modes)

The ServiceAccount needs `get`/`list`/`watch` on `namespaces`, `pods`, and
...
Daily indices (`filebeat-fluentbit-YYYY.MM.DD`) are created automatically. No ILM/ISM
...
| Index prefix            | `filebeat-{tenant}`         | `filebeat-fluentbit`           |
...
| -------------------------- | ------------------- | ------------------ |
| `{{NAMESPACE}}`            | 1st positional arg  | Both templates     |
| `{{DEPLOY_NAME}}`          | namespace or `cluster` | Both templates  |
| `{{DOCKER_IMAGE}}`         | 2nd positional arg  | DaemonSet template |
| `{{ES_HOST}}`              | 3rd positional arg  | DaemonSet template |
| `{{TENANT_LOCAL}}`         | `True` or `False`   | DaemonSet template |
| `{{NODE_SELECTOR_TENANT}}` | tenantname line or empty | DaemonSet template |
| `{{TOLERATIONS}}`          | mode-dependent block | DaemonSet template |

`--cluster-wide` sets `DEPLOY_NAME=cluster`, `TENANT_LOCAL=False`, removes the
tenantname nodeSelector, and uses a catch-all toleration. Without the flag,
`DEPLOY_NAME` equals the namespace and tenant-scoped defaults apply.
...
| --------------------- | --------------- | -------------------------------------------- |
| `FLUSH_INTERVAL`      | fluent-bit.conf | Flush interval in seconds (default `5`)      |
...
| `ES_WORKERS`          | fluent-bit.conf | ES output workers (default `1`)              |
