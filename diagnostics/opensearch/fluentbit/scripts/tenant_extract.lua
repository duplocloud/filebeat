...
local AGENT = freeze({ name = HOSTNAME, type = "fluent-bit", version = "4.2.3", hostname = HOSTNAME })
...
function extract_tenant_and_set_index(tag, timestamp, record)
    ...
    elseif SVC_LEVEL == "yes" and container_name ~= "unknown" then
        record["index_prefix"] = INDEX_PREFIX .. tenant .. "-" .. container_name
    else
        record["index_prefix"] = INDEX_PREFIX .. tenant
    end
